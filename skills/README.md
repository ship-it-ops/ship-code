...
| [ship-secure-code](ship-secure-code/) | Application-security review across 12 categories (SEC1-SEC12: auth, input validation, injection, XSS/output encoding, CSRF/origin, crypto, secrets, supply chain, PII/logging, resource exhaustion, path traversal, deserialization/SSRF) for Python, TypeScript/JavaScript, and Java. The depth target for `ship-reviewed-prs` SC delegation. |
| [ship-reviewed-prs](ship-reviewed-prs/) | Perform a thorough, multi-persona pull-request review (senior engineer, senior security, senior infra/SRE, conditional senior data engineer, conditional senior frontend engineer, plus test-coverage signal). Reads existing PR comment threads and suppresses already-resolved or won't-fix findings. Computes a deterministic APPROVE / REQUEST_CHANGES / COMMENT decision and submits via `gh` CLI — interactive in local mode, fully automated in CI. |
