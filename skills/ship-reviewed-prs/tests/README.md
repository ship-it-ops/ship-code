...
| `fixture-7-suggestions-only-approve` | v2.0.0 matrix: a single P6 (SE6-LINK-ROT-RISK) on a docs PR does NOT block APPROVE. Verifies the "Suggestions (improve when convenient)" caveat is emitted inside the APPROVE body and that `--auto-approve` would still be blocked locally despite the green-path. |
| `fixture-8-pending-ci-approve` | v2.0.0 matrix: pending CI checks (CodeQL language jobs irrelevant to a markdown diff) do NOT block APPROVE. Verifies the "Awaiting CI" caveat lists pending check names and that the inline-less APPROVE uses the simpler `gh pr review --approve --body` submission form. |
