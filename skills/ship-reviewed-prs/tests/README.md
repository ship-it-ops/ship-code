...
| `fixture-7-suggestions-only-approve` | Updated matrix: a single P6 (SE6-LINK-ROT-RISK) on a docs PR does NOT block APPROVE. Verifies the "Suggestions (improve when convenient)" caveat is emitted inside the APPROVE body and that `--auto-approve` would still be blocked locally despite the green-path. |
| `fixture-8-pending-ci-approve` | Updated matrix: pending CI checks (CodeQL language jobs irrelevant to a markdown diff) do NOT block APPROVE. Verifies the "Awaiting CI" caveat lists pending check names and that the inline-less APPROVE uses the simpler `gh pr review --approve --body` submission form. |
| `fixture-9-devops-delegation` | IN persona's depth target = `ship-devops`. PR ships workflow + Dockerfile + manifest + service code. Direct-emit IN findings on high-precision single-line hits PLUS a single `Run /ship-devops on services/<name>/` delegation bullet for the compound multi-file pipeline review. Verifies compound finding tags `[IN1 / DEV2.1-NO-ROLLBACK]` surface the depth-target's category alongside the orchestrator's priority code. |
